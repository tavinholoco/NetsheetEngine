...
**Última atualização:** 08/08/2026 — **revisão v1.12**: Fases 0, 1, **2 e 3 concluídas** (T3.1–T3.5 em 08/08/2026). Fase 3 entregue: salas persistidas em `rooms.room_state jsonb` (service-role, debounce 2s, restauração no boot, shutdown gracioso), **reconexão com mesmo peerId** (ficha preservada por last-write-wins, 1 token por jogador, auto-re-join no cliente), **timeout de isOnline** com heartbeat + broadcast ao vivo, e **T3.5 decidida**: transporte segue SSE (avaliação WS/Yjs na Fase 5) e formato de persistência **JSON simples** (servidor autoritativo não precisa de CRDT — sem retrabalho futuro).
