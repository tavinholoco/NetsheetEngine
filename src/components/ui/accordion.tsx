...
>(({ className, children, ...props }, ref) => (
  <AccordionPrimitive.Header className="flex">
    <AccordionPrimitive.Trigger
      ref={ref}
      className={cn(
        "flex flex-1 items-center justify-between py-4 font-bold font-mono text-xs uppercase tracking-wider text-slate-200 transition-all hover:text-cyan-400 [&[data-state=open]>svg]:rotate-180",
        className
      )}
      {...props}
    >
...
>(({ className, children, ...props }, ref) => (
  <AccordionPrimitive.Content
    ...
    {...props}
  >
    <div className={cn("pb-4 pt-0 text-slate-300 font-sans", className)}>{children}</div>
  </AccordionPrimitive.Content>
));
